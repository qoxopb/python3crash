...
with open('member.dat')as f:     #모두 읽기
    doc1 = f.read()
print(doc1)


# 여러건의 회원정보가 저장된 파일 읽어오기 2 (추천!)
with open('member.dat')as f:     #행단위로 모두 읽기
    doc2 = f.readlines()
...
with open('member.dat')as f:     #데이터 중 한 행 읽기
    doc3 = f.readline()
print(doc3)                      #한 건만 출력
...
for doc in doc2:
    ...
    print(row, end='')           # 행의 끝에 줄바꿈 있으므로 end=''사용
...
for sj in sjs:
    item = sj.split(', ')           #split(구분자)
    row = (f'이름: {item[0]}국어: {item[1]}영어: '
           f'{item[2]}수학: {item[3]} ')
    print(row, end='')
