...
result = '흑자' if (income > outcome) else '적자'   # 조건문에 () -선택

print(f'{income} {outcome} {result}')


# 재난지원금 대상자 판별  (22 - 76p)
# 월 소득과 지원금 수령여부 입력받아 수급대상 여부 출력
# 조건 1 : 소득기분 4000000 이하
# 조건 2 : 다른 지원금 없는 경우
income = int(input('월 소득 : '))
isTake = int(input('타 지원금 수령 여부 - 예: 1, 아니오: 2'))
result = '수급 대상자' if (income < 4000000 and isTake == 2) else '수급 대상자 외'
print(f'{income} {isTake} {result}')

income = int(input('월 소득 : '))
isTake = int(input('타 지원금 수령 여부 (예: 1, 아니오: 2)'))
isTarget=  (income < 4000000) and (isTake == 2)      # 조건문 분리

result = '수급 대상자' if isTarget else '수급 대상자 외'
print(f'{income} {isTake} {result}')
...
import operator as op       #모듈사용선언, as구문 사용해 별칭지정가능

# isTarget=  (income < 4000000) and (isTake == 2)       #주석으로 원래구문 표시하여 가독성높임
isTarget=  op.and_((op.le(income, 4000000),
                          op.eq(isTake, '2')))


# (22 - 81p) 3대의 컴퓨터로 8시간 업무가 기준
# 근무시간 입력 시 컴퓨터수량 출력
# pc * time = 3 * 8
# pc = (3 * 8) / time

wtime = int(input('근무시간: '))
coms = op.truediv(3* 8, wtime)
print(f'필요한 컴퓨터 대수 : {coms:.0f}')      # .0f 소수점 올림
