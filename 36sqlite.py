...
# sqlite 설치법
# sqlite-tools-win-x64-3440200.zip  (2023.1.04기준)
...
# 데이터  삽입: insert
# insert into member values('abc123', 'qwer', 'asdf', 'zcxv');
...
# tsv 파일 가져오기 import (기존db라면 열려있는지 확인)
# .mode tabs
# .import tsv파일명 테이블명

# .import c:/java/zipcode_20130201.txt zipcode2013
# select * from zipcode2013;

# 단, import시 csv, tsv 파일의 인코딩은 UTF-8이어야함.
...
# 실행할 sql 문을 작성하고
# 실행후 결과를 커서에 저장
sql = 'select * from zipcode20132 where dong like "자양1%"'
cursor.execute(sql)
...
# 작업이 끝나면 커서와 접속을 닫음  (열기 conn cursor - 닫기 cursor conn)
cursor.close()
conn.close()
...
cursor = conn.cursor()
sql = 'select * from employees where employee_id like "1%"'
cursor.execute(sql)
while True:
    row = cursor.fetchone()
    if not row: break
...
sql = (' select job_id, count() as emps from employees '            #' ' 내용 처음 끝 공백추가(오류방지)
       ' group by job_id order by emps desc ')                      # sql 콘솔 내에서만 ; 로 종결
cursor.execute(sql)
