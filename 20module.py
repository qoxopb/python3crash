...
# C:\Users\clouds\py39\Lib\site-packages
...
# 모듈의 종류
# 내장(표준 builtin) 모듈 : 파이썬에서 기본으로 제공하는 모듈
# 사용자 정의 모듈: 직접 개발자/조직/회사 가 만든 모듈
...
os.listdir('c:\Java\python39')      # 지정 디렉토리의 내용 출력(파일탐색기 구현가능)



# 시간, 날짜 패키지
# 시간과 관련된 정보를 다루는 함수가 있는 패키지
# time : 현재시간을 실수형태로 출력 (1970.1.1 기준 - 유닉스 가동시작시점)
...
fmt1 = '%Y-%m-%d %H:%M:%S %a'      # 24시간          #yearmonthday  HourMinuteSecond
fmt2 = '%Y-%m-%d %p %I:%M:%S %A'   # 12시간          # %p %I  오전오후
fmt3 = '%j %w %W'                  #누적일 요일 누적주
fmt4 = '%x %X'                     # 날짜 시간

# strftime(stringformattime. 형식문자열, 날짜시간정보)
...
# 현지화 (localization)
# 기존에 이미  개발된 프로그램을
# 특정 지역, 언어에 맞춰 다시 설계, 개발하는 것


# datetime 모듈
# time 모듈과 달리 시간대 timezone 를 이용한
# 현지시간 출력, 날짜/시간차이 계산기능 추가
...
print(calendar.weekday(2023,12,28))   #월요일 : 0

print(calendar.isleap(2023))    #윤년 여부
...
person = {'name': '이름', 'addr': '서울 관악구',
          'age': 32, 'email': 'abc123@987xyz'}
print(person)
...
with open('person.pickle', 'wb') as f:     #wb 쓰기권한
    # dump(객체명, 대상)
...
person = ''
os.chdir('c:\Java\python39')
with open('person.pickle', 'rb') as f:      # rb 읽기권한
    # load(대상)
