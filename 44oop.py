...
class Sungukservice:
    ...
    @staticmethod     # 정적 메서드 : 객체 선언없이 바로 사용가능한 메서드
                      # 호출방법 : 클래스명.함수명
                      # 정적 메서드로 정의된 함수에는 self 지정하지 않음
    def read_sungjuk():
        name = input('이름은 ?')
        kor = int(input('국어는 ?'))
        eng = int(input('영어는 ?'))
...
sjsrv.compute_sunjuk(sj)
print(sj)


# 성적서비스 호출2
sj2 = Sungukservice.read_sungjuk()
Sungukservice.compute_sunjuk(sj2)
print(sj2)
