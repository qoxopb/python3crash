...
# 대입식을 한줄에 나타내려면 ;(세미콜론) 사용
d = 10; e = 20; f = 30
print(d, e, f)

# 다중 할당 : 여러 변수의 값을 한번에 할당 - ,(콤마)사용
g, h, i = 10, 20, 30
print(g, h, i)

# 정수 대입시 자릿수를 의미하는 구분자 지정 :  _ 사용, print 시에는 미적용
j = 1_000_000_000   #10억
print(j)

# 상수 정의하기
# 프로그램 실행 중에 언제나 동일한 값을 유지하는 변수 constant
# 상수의 이름은 대문자로 표기
PI = 3.14152
print(PI)
...
# 논리식의 경우, 수식의 구성에 따라 단축식 평가short-circuit가 가능   (단축식 평가 : 첫 요소 값에 따라 이하 계산 생략)
print((5 == 3) & (5 > 3))      # and
print((5 != 3) | (5 > 3))      # or
...
# 다음 수식의 결과는?
print(0 and 'abc', 1 and 'abc')    # 문자열에 논리연산자 적용
...
# % 서식 - 서식문자열 사용    (문자-s, 숫자-d
print('이름 : %s, 나이 : %d' % (name, age))

# .format 사용         (*args : 자동표시. 가변변수)
